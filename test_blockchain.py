...
def test_difficulty_decreases(blockchain):
    ...
    for i in range(10):
        ...
        blockchain.add_transaction(sender="0", recipient="Devin", amount=1)
        previous_hash = last_block.hash
        # Calculate new timestamp for the new block
        new_timestamp = blockchain.chain[-1].timestamp + 20
        new_block = Block(index=last_block.index + 1, transactions=blockchain.current_transactions, previous_hash=previous_hash, proof=proof, timestamp=new_timestamp)
